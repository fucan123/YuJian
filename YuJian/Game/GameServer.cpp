...
void GameServer::Open(const char* data, int len)
{
	...
	if (m_pGame->GetOnLineCount() >= MAX_ACCOUNT_LOGIN) {
		...
		LOGVARP2(log, "red", "目前只支持5个账号，敬请期待！！！");
		return;
	}	
...
{
	return;
	LOGVARN(64, "%s 邀请进入副本[%d]", p->Name, p->Socket);
	printf("%s 邀请进入副本 [%d]\n", p->Name, p->Socket);
...
void GameServer::OnClose(SOCKET client, int index)
{
	...
	if (p && p->Flag == 1) {
		...
		if (self->m_pGame->CheckStatus(p, ACCSTA_COMPLETED)) {  // 已完成
			...
			else if (self->m_pGame->m_Setting.LogoutByGetXL) { // 已领完
